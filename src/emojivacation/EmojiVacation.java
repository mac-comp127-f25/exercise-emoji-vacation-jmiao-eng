...
public class EmojiVacation {
    ...
    private static void generateVacationPhoto(CanvasWindow canvas) {
        ...
        if(percentChance(50)) {
            double size = randomDouble (120,200);
            int layers = randomInt(1,3);
            double baseY = randomDouble(320, 420);
        ...
        if(percentChance(60)){
            ...
            addForest(canvas, 380, 180, count);
        }
    ...
    private static void positionFamily(
        ...
        for(GraphicsGroup person : family) {
            double yTop = baselineY - person.getHeight();
            person.setPosition(x, yTop);
